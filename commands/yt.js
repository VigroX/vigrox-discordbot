...
const queue = new Map();
try {
	var yt = new YouTube(process.env.YOUTUBE_API_KEY);
} catch(e) {
	console.log("Couldn't find the environment variable: YOUTUBE_API_KEY");
	var yt = false;
}
exports.run = async (client, message, args) => {
	if(!yt)
		return message.reply(":no_entry_sign: This module couldn't connect to API key!");
	if(!client.cjson.modules.yt)
		return message.reply(":no_entry_sign: This module is disabled!");
	if(message.guild.roles.find(role => role.name === "DJ")) {
	...
	switch(args[0]) {
			...
			if(url.match(/youtu\.be\/([^&\n?#]+)/)) {
				...
				try {
					...
					try {
						...
						if(err.errors[0].reason === "keyInvalid") {
							var yt = false;
							return message.reply(":no_entry_sign: This module couldn't connect to API key!");
						}
		...
		case "playlist":
			...
			message.channel.send(":arrows_counterclockwise: Adding playlist songs...");
			let pl = await yt.getPlaylist("https://www.youtube.com/playlist?list=" + YT_PLAYLIST_ID);
...
function play(guild, song) {
	...
	const streamOptions = { audioonly: true, passes: 2, bitrate: "auto" };
